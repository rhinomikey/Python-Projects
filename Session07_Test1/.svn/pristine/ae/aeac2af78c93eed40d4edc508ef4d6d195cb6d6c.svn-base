...
def problem2(circle, rectangle, window):
    ...
    circle_rect = circle.get_bounding_box()
    circle_center = circle_rect.get_center()

    rect_center = rectangle.get_center()
    rect_height = rectangle.get_height()

    line = rg.Line(rg.Point(rectangle.get_center().x, rectangle.get_center().y),
                    rg.Point(circle_center.x, circle_center.y))
    line.thickness = 10
    line.color = 'red'

    line1 = rg.Line(rg.Point(rect_center.x, rect_center.y + rect_height / 2),
                    rg.Point(rect_center.x, rect_center.y - rect_height / 2))

    line1.thickness = rectangle.outline_thickness
