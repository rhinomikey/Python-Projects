...
def problem2b(rectangle, n, delta, win):
    ...
    point = rg.Point(rectangle.get_upper_left_corner().x,
                     rectangle.get_upper_left_corner().y)
    point1 = rg.Point(rectangle.get_lower_right_corner().x,
                      rectangle.get_lower_right_corner().y)

    for i in range(n):
        rect2 = rg.Rectangle(rg.Point(point.x - (delta / 2) * i, point.y - (delta / 2) * i),
                             rg.Point(point1.x + (delta / 2) * i, point1.y + (delta / 2) * i))
        rect2.attach_to(win)
