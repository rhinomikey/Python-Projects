...
#
# NOTE: You need to train the Pixy to track a color for it to work.
...
def test_camera():
    """ Connects to a RoseBot.  Demonstrates the use of its camera. """
    # ------------------------------------------------------------------
    # Recall that this is how you make a RoseBot and connect to it.
    # ------------------------------------------------------------------
    robot = rb.RoseBot()
    robot.connect_wifly(1)  # Use the number on YOUR robot
    
    # ------------------------------------------------------------------
    # Next, here is how you make noises:
    # ------------------------------------------------------------------
    robot.buzzer.play_tone(220)
    robot.connection.sleep(0.5)
    robot.buzzer.stop()
    
    robot.connection.sleep(1.5)
    ...
    # ------------------------------------------------------------------
    # Last, here is how you use the camera:
    # ------------------------------------------------------------------
    close_proximity = 500  # Change as necessary for YOUR robot
    
    while True:
        robot.connection.sleep(0.5)  # IMPORTANT TO HAVE THIS IN YOUR LOOPS!
        
        # Sometimes you want ALL the blocks to iterate through them, like this:
        blocks = robot.camera.get_blocks()
        
        if len(blocks) == 0:
            print("No blocks found")
        else:
            for k in range(len(blocks)):
                print("Here is the data you have, for Block " + str(k) + ":")
                print("  x value = ", blocks[k].x)
                print("  y value = ", blocks[k].y)
                print("  width value = ", blocks[k].width)
                print("  height value = ", blocks[k].height)  
                print("  size = ", blocks[k].size())
        
        # But more often, you want only the BIGGEST block, as in
        # the following.  Hover over it to see the optional parameters:
        block = robot.camera.get_block()
        if block is not None:
            print(block.x)  # And similarly for other instance variables.
        
        # Quit this program when you put a hand (or other object)
        # close to the left proximity sensor:
        if robot.left_proximity_sensor.read() > close_proximity:
            break
...
########################################################################
# The rest of this module is from your previous robot session.
# Use it for examples as needed.
########################################################################
...
def test_blink():
    ...
    # ------------------------------------------------------------------
    robot = rb.RoseBot()
    robot.connect_wifly(22)  # Use the number on YOUR robot
...
def test_move():
    ...
    # ------------------------------------------------------------------
    robot = rb.RoseBot()
    robot.connect_wifly(12)  # Use the number on YOUR robot
