...
def bullseye(window, colors):
    ...
    circle = rg.Circle(center,height/2 - 25)
    circle.fill_color = colors[0]
    circle.attach_to(window)
    radius = circle.radius
    del_radius = (radius-50)/len(colors)
    circles = rg.Circle(center,50)
    circles.fill_color = colors[-1]
    circles.attach_to(window)
    for i in range(1,len(colors)):
        radius = radius- del_radius
        circles = rg.Circle(center,radius)
        circles.fill_color = colors[i]
        circles.attach_to(window)
    window.render()
