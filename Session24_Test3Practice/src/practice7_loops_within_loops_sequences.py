...
def test_integers():
    ...
    expected = [5, 6, 3, 16, 19, 3, 7, 3, 8, 90, 65]
    answer = integers([(5,6,3),
                       (16, 'blah', 19),
                       [9.1,3,7,3,8],
                       'hello',
                       [],
...
def test_big_letters():
    ...
    expected = 'WIOHIAGAINBPPPOBEETLESSCWDWOOF'
    answer = big_letters([(3, 1, 4),  # not a string
                          'What Is going On?',  # OTSSS
                          (10, 'This is not a String', 10),  # not a string
                          [],  # not a string
                          ['HI'],  # not a string
                          'HI AGAIN',  #
                          ['Blah'],  # not a string
...
def big_letters(sequence_of_sequences):
    ...
    for i in range(len(seq)):
        if type(seq[i]) == str:
            uppers = uppers + [j for j in seq[i] if j.isupper()]
    return ''.join(uppers)
