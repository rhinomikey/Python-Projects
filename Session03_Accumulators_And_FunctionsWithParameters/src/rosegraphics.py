...
#                      (title, (Color, str)
    ...
    def get_next_mouse_click(self):
        """
        Waits for the user to click in the window.  Then returns
        the rg.Point that represents the point where the user clicked.

        For example, if this method is called and then the user clicks
        near the upper-right corner of a 300 x 500 window,
...
class _Shape(object, metaclass=__FreezeClass__):
    ...
    def attach_to(self, rose_canvas):
        ...
        on that RoseCanvas.
        """
        if isinstance(rose_canvas, RoseWindow):
...
class _RectangularShape(_Shape):
    ...
    def __repr__(self):
        f_string = ''
        f_string += '{}: corner1=({}, {}), corner2=({}, {}), fill_color={}, '
...
class Circle(_ShapeWithCenter, _ShapeWithOutline):
    ...
    It can attach itself to a window by attach_to, change its center
    by   move_by  and  move_to, use clone  to make a copy of itself,
    and get its bounding box by get_bounding_box.
    
    To construct a Circle, use:
       rg.Circle(center, radius)
    where  center  is an rg.Point object and  radius  is a positive
    integer.  For example:  rg.Circle(rg.Point(100, 75), 30)

    Instance variables include:

      center:  An rg.Point that specifies the center of the Circle.
      
      radius:  The radius of the Circle.
      
      fill_color:  The Circle is filled with this color (e.g. 'green').
      
      outline_color:  The outline of the Circle is this color.
      
      outline_thickness:  The thickness (in pixels) of the outline
        of the Circle.
    
    Examples:
       center = rg.Point(100, 75)
       circle = rg.Circle(center, 30)
       
       circle.fill_color = 'blue'
       cicle.outline_color = 'black'
       circle.outline_thickness = 5
       
       window = rg.RoseWindow()
       circle.attach_to(window)
       
       circle.move_to(300, 200)
...
class Ellipse(_RectangularShape, _ShapeWithOutline):
    ...
    where  corner1  and  corner2  are rg.Point objects that specify
    opposite corners of the bounding box within which the Ellipse
    is drawn.  For example:
      rg.Ellipse(rg.Point(100, 50), rg.Point(300, 200))
    specifies the ellipse that fits inside a rectangle whose
    upper-left corner is (100, 50) and whose lower-right
    corner is (300, 200).

    Instance variables include:

      corner1:  An rg.Point that specifies one corner
      of the bounding box of the Ellipse.
      
      corner2:  An rg.Point that specifies an opposite corner
      of the bounding box of the Ellipse.
      
      fill_color:  The Ellipse is filled with this color (e.g. 'green').
      
      outline_color:  The outline of the Ellipse is this color.
...
class Line(_Shape, _ShapeWithThickness):
    ...
    It can attach itself to a window by attach_to, change its
    endpoints by   move_by, use clone  to make a copy of itself,
    and get its midpoint by   get_midpoint.
    
    To construct a Line, use:
       rg.Line(start, end)
    where  start  and  end   are rg.Point objects that are
    the endpoints of the Line.  For example:
       rg.Line(rg.Point(100, 50), rg.Point(200, 30))

    Instance variables include:

      start:  The rg.Point that is one end of the Line.
      
      end:  The rg.Point that is the other end of the Line.

      color:  The Line is drawn with this color.
            
      thickness:  The thickness (in pixels) of the Line.
      
      arrow:  Specifies whether or not the Line is drawn as an
        arrow.  Possible values are: None, 'first', 'last', 'both'.
    ...
    # Mark: you MUST use this form to declare properties
    # to get Eclipse to treat it like a variable.
    # If you use the decorators (@ property and @ arrow.setter), Eclipse will
    # INCORRECTLY think it is a method and autocomplete it with () at the end.
...
class Point(_Shape, _ShapeWithOutline):
    ...
    Instance variables include:
      ...
      fill_color:  The Point is filled with this color (e.g. 'green').
      
      outline_color:  The outline of the Point is this color.
    ...
    Examples:
       ...
       p.move_by(-50, 60)
    """
    defaults = {'width_for_drawing': 3,
                'height_for_drawing': 3,
                'fill_color': 'black'}
...
class Rectangle(_RectangularShape, _ShapeWithOutline):
    ...
    opposite corners of the rectangle.  For example:
      rg.Rectangle(rg.Point(100, 50), rg.Point(300, 200))
    specifies the rectangle whose upper-left corner is (100, 50)
    and whose lower-right corner is (300, 200).
    ...
    Instance variables include:
      ...
      corner2:  An rg.Point that specifies an opposite corner
      of the bounding box of the Rectangle.
      
      fill_color:  The Rectangle is filled with this color (e.g. 'green').
      
      outline_color:  The outline of the Rectangle is this color.
