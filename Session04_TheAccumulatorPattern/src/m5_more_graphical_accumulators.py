...
def draw_squares_from_circle(n, circle, window):
    ...
    for i in range(n + 1):
        center = rg.Point(square.center.x + circle.radius * i, square.center.y + circle.radius * i)
        square1 = rg.Square(center, circle.radius * 2)
...
def test_draw_circles_from_rectangle():
    ...
    point2 = rg.Point(270, 270)
    rectangle = rg.Rectangle(point1, point2)
    draw_circles_from_rectangle(4, 2, rectangle, window)

    point1 = rg.Point(150, 150)
    point2 = rg.Point(140, 140)
    rectangle = rg.Rectangle(point1, point2)
    draw_circles_from_rectangle(5, 5, rectangle, window)
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    circle_center = rg.Point(center.x + (height + (width + 3)) / 2, center.y)
    circle = rg.Circle(circle_center, height / 2)
    circle.fill_color = 'blue'

    circle2_center = rg.Point(center.x, center.y + ((height / 2) + (width / 2)) + 2)
    circle2 = rg.Circle(circle2_center, width / 2)
    ...
    for i in range(m):
        x = circle.center.x + circle.radius * (2 * i)
        point = rg.Point(x, circle.center.y)
        circle1 = rg.Circle(point, height / 2)
        circle1.fill_color = 'blue'
        circle1.attach_to(window)

    for i in range(n):
        y = circle2.center.y + circle2.radius * 2
        point1 = rg.Point(circle2.center.x, y)
        circle2 = rg.Circle(point1, width / 2)
        circle2.fill_color = 'blue'
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    centerx1 = rectangle1.get_center().x
    centery1 = rectangle1.get_center().y

    centerx2 = rectangle2.get_center().x
    centery2 = rectangle2.get_center().y

    rectangle1.attach_to(window)
    rectangle2.attach_to(window)

    for i in range(n):
        line = rg.Line(rg.Point(centerx1, centery1), rg.Point(centerx2, centery2))
        line.thickness = 5
        ...
        centerx1 -= rectangle1.get_width() / 2
        centery1 += rectangle1.get_height() / 2

        centerx2 -= rectangle1.get_width() / 2
        centery2 += rectangle1.get_height() / 2
